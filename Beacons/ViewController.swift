...
class ViewController: UIViewController {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        places = RealmHelper.retrievePlace()
        showPOI()
    ...
    func showPOI() {
        for (place, info) in sfPlaces {
         ...
         //   let isVisited =  places?.contains({ (place) -> Bool in
         //       return place.placeId == info[0]
          //  })
            
            if let places = places {
                for item in places {
                    if item.placeId == info[0] {
